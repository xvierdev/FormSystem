...
class Pergunta(db.Model):
    ...
    alternativas = db.relationship(
        'Alternativa', backref='pergunta', cascade="all, delete-orphan")
    resposta_correta = db.relationship(
        'Alternativa', primaryjoin="and_(Pergunta.id==Alternativa.pergunta_id, Alternativa.correta==True)", uselist=False, viewonly=True, lazy='joined')
...
class Submissao(db.Model):
    ...
    respostas = db.relationship(
        'RespostaAluno', backref='submissao', cascade="all, delete-orphan")


class RespostaAluno(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    submissao_id = db.Column(db.Integer, db.ForeignKey(
        'submissao.id'), nullable=False)
    pergunta_id = db.Column(db.Integer, db.ForeignKey(
        'pergunta.id'), nullable=False)
    alternativa_escolhida_id = db.Column(
        db.Integer, db.ForeignKey('alternativa.id'), nullable=True)
...
# --- Funções de Apoio e Rotas (sem alterações até a rota /relatorio) ---
# ... (código existente de load_user, carregar_questionario_do_csv, login, logout, etc.)
...
# ... (Rotas de login, logout, upload, configuracoes, index, iniciar_questionario, questionario, submeter_questionario - sem alterações)
...
def configuracoes():
    if request.method == 'POST':
        ...
        new_password, confirm_password = request.form.get(
        ...
        existing_user = Admin.query.filter(
            Admin.username == new_username, Admin.id != user.id).first()
        if existing_user:
...
def submeter_questionario():
    ...
    if not submissao:
        return redirect(url_for('index'))
    perguntas_db = Pergunta.query.options(
        selectinload(Pergunta.alternativas)).all()
    alternativas_map = {
        alt.id: alt for p in perguntas_db for alt in p.alternativas}
        ...
        if id_alternativa_str and id_alternativa_str.isdigit():
            ...
            if id_alternativa in alternativas_map:
                ...
                if alternativa_escolhida.correta:
                    correta = True
                    total_acertos += 1
        respostas_para_salvar.append(RespostaAluno(submissao_id=submissao.id, pergunta_id=pergunta.id,
                                     alternativa_escolhida_id=alternativa_escolhida.id if alternativa_escolhida else None, correta=correta))
...
@login_required
def relatorio():
    # ... (código existente da rota /relatorio - sem alterações)
    submissoes = Submissao.query.filter_by(concluida=True).options(selectinload(Submissao.respostas).options(joinedload(RespostaAluno.pergunta).options(
        joinedload(Pergunta.resposta_correta)), joinedload(RespostaAluno.alternativa_escolhida))).order_by(Submissao.data_hora.desc()).all()
...
def limpar_registros():
    try:
        ...
        summary_logger.info(log_message)
        flash(
            f'Todos os {num_submissoes} registros de alunos foram apagados com sucesso!', 'success')
    except Exception as e:
        db.session.rollback()
        summary_logger.error(
            f"LIMPEZA DE DADOS FALHA - Admin: {current_user.username} - Erro: {e}")
...
@login_required
def estatisticas():
    perguntas = Pergunta.query.options(
        selectinload(Pergunta.alternativas)).all()
    respostas = RespostaAluno.query.all()

    # 1. Preparar dados para os gráficos de barras
    charts_data = {}
    for p in perguntas:
    ...
    for r in respostas:
        if r.pergunta_id in charts_data and r.alternativa_escolhida:
            try:
                ...
                idx = charts_data[r.pergunta_id]['labels'].index(
                    r.alternativa_escolhida.texto)
                charts_data[r.pergunta_id]['data'][idx] += 1
            except ValueError:
                # Caso a alternativa não seja encontrada (pouco provável)
                continue

    # 2. Calcular perguntas com mais acertos e erros
    ...
    for r in respostas:
        if r.correta:
            acertos_por_pergunta[r.pergunta_id] += 1
        else:
            erros_por_pergunta[r.pergunta_id] += 1

    mais_acertos = None
    if acertos_por_pergunta:
        id_mais_acertos = max(acertos_por_pergunta,
                              key=acertos_por_pergunta.get)
        pergunta = Pergunta.query.get(id_mais_acertos)
        mais_acertos = {
            'texto': pergunta.texto,
            'contagem': acertos_por_pergunta[id_mais_acertos]
        }
    ...
    if erros_por_pergunta:
        ...
        mais_erros = {
            'texto': pergunta.texto,
            'contagem': erros_por_pergunta[id_mais_erros]
        }

    return render_template('estatisticas.html',
                           charts=charts_data,
                           mais_acertos=mais_acertos,
                           mais_erros=mais_erros,
                           total_respostas=total_respostas)


# --- Rotas de Exportação e Inicialização (sem alterações) ---
# ... (código existente de get_todas_submissoes, exportar_simplificado, exportar_completo e if __name__ == '__main__')
...
def exportar_simplificado():
    ...
    for submissao in submissoes:
        data_hora = submissao.data_hora.replace(tzinfo=pytz.utc).astimezone(
            SAO_PAULO_TZ).strftime('%d/%m/%Y %H:%M:%S')
...
def exportar_completo():
    ...
    headers = ["Nome do Aluno", "Pergunta",
               "Resposta do Aluno", "Resposta Correta", "Acertou?"]
    ws.append(headers)
    for cell in ws[1]:
        cell.font = Font(bold=True)
    for submissao in submissoes:
        for resposta in submissao.respostas:
            ...
            ws.append([submissao.nome_aluno, resposta.pergunta.texto,
    ...
    with app.app_context():
        ...
        if Pergunta.query.count() == 0:
            ...
            try:
                ...
                print(
                    "AVISO: 'mario.csv' não encontrado. O sistema iniciará sem perguntas.")
